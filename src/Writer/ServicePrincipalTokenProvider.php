...
/**
 * Mints a Microsoft Entra ID (Azure AD) access token for a Service Principal using the
 * OAuth2 client_credentials flow and stores it in a file consumable by BCP (`-G -P <file>`).
 */
class ServicePrincipalTokenProvider
{
    ...
    public function getAccessToken(): string
    {
        ...
        if (!is_array($data) || !isset($data['access_token']) || !is_string($data['access_token'])) {
            throw new UserException('Microsoft Entra ID token response did not contain an access token.');
        }
    ...
    protected function sendTokenRequest(string $url, array $body): string
    {
        $ch = curl_init($url);
        if ($ch === false) {
            throw new UserException('Unable to initialize the Microsoft Entra ID token request.');
        }
        ...
        if ($response === false) {
            throw new UserException(sprintf('Failed to request Microsoft Entra ID token: %s', $error));
        }

        if ($statusCode < 200 || $statusCode >= 300) {
            throw new UserException(sprintf(
                'Failed to obtain Microsoft Entra ID token (HTTP %d): %s',
                $statusCode,
                (string) $response,
            ));
        }
